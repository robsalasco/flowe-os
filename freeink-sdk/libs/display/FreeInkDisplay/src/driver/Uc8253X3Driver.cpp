...
const Uc8253X3Config& uc8253X3DefaultConfig() {
  static const Uc8253X3Config cfg = {
      ...
      {lut_x3_vcom_flash, lut_x3_ww_flash, lut_x3_bw_flash, lut_x3_wb_flash, lut_x3_bb_flash},
...
void Uc8253X3Driver::displayWindow(EpdBus& bus, const uint8_t* fb, const uint8_t* prev, uint16_t x, uint16_t y,
  ...
  (void)prev;
  displayWindowWithBank(bus, fb, x, y, w, h, _cfg.fast, turnOff);
}

void Uc8253X3Driver::displayWindowFlash(EpdBus& bus, const uint8_t* fb, uint16_t x, uint16_t y, uint16_t w,
                                        uint16_t h) {
  displayWindowWithBank(bus, fb, x, y, w, h, _cfg.flash, /*turnOff=*/false);
}

void Uc8253X3Driver::displayWindowWithBank(EpdBus& bus, const uint8_t* fb, uint16_t x, uint16_t y, uint16_t w,
                                           uint16_t h, const Uc8253LutBank& bank, bool turnOff) {
  if (!fb || w == 0 || h == 0 || x >= _w || y >= _h) return;
  // A windowed differential needs DTM1 to hold the displayed BW frame. Fall
  // back to full-frame FAST when it can't (same predicate family as
  // displayGrayscaleBase's cleanBaseNeeded), plus panel-off: display() owns
  // the wake transition (it upgrades to the stronger Half waveform).
  const bool cleanBaseNeeded = _inGrayscaleMode || !_redRamSynced || _grayState.lsbValid || _forceFullSyncNext ||
                               _initialFullSyncsRemaining > 0 || !_isScreenOn;
  if (cleanBaseNeeded) {
    display(bus, fb, nullptr, RefreshMode::Fast, turnOff);
    return;
  }

  // Window geometry, exactly like preconditionGrayscale: X byte-aligned
  // outward (PTL horizontal resolution is 8 px), Y converted to GATE space
  // (logical row y lives at gate H-1-y).
  ...
  const uint16_t byteStart = xs / 8;
  const uint16_t bytesPerRow = static_cast<uint16_t>((xe - xs + 1) / 8);
  ...
  // New frame into DTM2, window bytes only, rows bottom-first so they land at
  // the same gates the full-frame sendPlaneFlipped write uses.
  bus.cmd(CMD_DTM2);
  bus.beginTxn();
  for (int r = static_cast<int>(yEndLogical); r >= static_cast<int>(y); r--) {
    bus.rawWriteBytes(fb + static_cast<uint32_t>(r) * _wb + byteStart, bytesPerRow);
  }
  bus.endTxn();
  // Full-voltage differential inside the window (CDI 0x29). `bank` is _fast
  // for the standard partial, or the ~8-frame _flash for press feedback.
  loadBankCdi(bus, 0x29, 0x07, bank);
  triggerRefresh(bus, turnOff);
  // DTM1 window resync so the next differential (full-frame or windowed)
  // diffs against what is actually on glass.
  bus.cmd(CMD_DTM1);
  bus.beginTxn();
  for (int r = static_cast<int>(yEndLogical); r >= static_cast<int>(y); r--) {
    bus.rawWriteBytes(fb + static_cast<uint32_t>(r) * _wb + byteStart, bytesPerRow);
